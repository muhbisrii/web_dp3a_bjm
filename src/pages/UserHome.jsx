    ...
    <div className="flex h-screen bg-gradient-to-br from-slate-50 to-slate-200 font-sans overflow-hidden relative">
      ...
      <aside className={`fixed md:relative z-30 w-64 h-full bg-[#0f172a] text-white flex flex-col transition-transform duration-300 shadow-xl ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className="p-4 border-t border-slate-800">
          <button onClick={handleLogoutClick} className="w-full flex items-center space-x-3 px-4 py-3 text-red-400 hover:bg-red-900/20 rounded-xl transition-colors"><LogOut size={20} /><span className="text-sm font-medium">Keluar Akun</span></button>
        </div>
      </aside>
          ...
          <div className="max-w-5xl mx-auto pb-10">
            ...
            {view === 'home' && (
              <motion.div key="home" variants={pageVariants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.3 }} className="space-y-8">
                 ...
                 <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                      <div className="bg-white/80 backdrop-blur-md p-5 rounded-2xl shadow-sm border border-white/60 flex flex-col"><div className="flex items-center justify-between mb-2"><span className="text-slate-500 text-xs font-bold uppercase tracking-wide">Total Laporan</span><div className="p-2 bg-slate-100 rounded-lg text-slate-600"><FileBarChart size={18}/></div></div><span className="text-3xl font-bold text-slate-800 mt-auto">{stats.total}</span></div>
                      <div className="bg-white/80 backdrop-blur-md p-5 rounded-2xl shadow-sm border border-white/60 flex flex-col"><div className="flex items-center justify-between mb-2"><span className="text-amber-600 text-xs font-bold uppercase tracking-wide">Menunggu</span><div className="p-2 bg-amber-50 rounded-lg text-amber-600"><Clock size={18}/></div></div><span className="text-3xl font-bold text-amber-600 mt-auto">{stats.menunggu}</span></div>
                      <div className="bg-white/80 backdrop-blur-md p-5 rounded-2xl shadow-sm border border-white/60 flex flex-col"><div className="flex items-center justify-between mb-2"><span className="text-blue-600 text-xs font-bold uppercase tracking-wide">Diproses</span><div className="p-2 bg-blue-50 rounded-lg text-blue-600"><Settings size={18}/></div></div><span className="text-3xl font-bold text-blue-600 mt-auto">{stats.diproses}</span></div>
                      <div className="bg-white/80 backdrop-blur-md p-5 rounded-2xl shadow-sm border border-white/60 flex flex-col"><div className="flex items-center justify-between mb-2"><span className="text-emerald-600 text-xs font-bold uppercase tracking-wide">Selesai</span><div className="p-2 bg-emerald-50 rounded-lg text-emerald-600"><CheckCircle size={18}/></div></div><span className="text-3xl font-bold text-emerald-600 mt-auto">{stats.selesai}</span></div>
                 </div>
              </motion.div>
            )}

            {view === 'form' && (
               <motion.div key="form" variants={pageVariants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.3 }} className="bg-white/80 backdrop-blur-md rounded-2xl shadow-lg border border-white/60 overflow-hidden">
                  <div className="bg-slate-50/50 px-8 py-6 border-b border-slate-200"><h2 className="text-2xl font-bold text-slate-800">Formulir Pengaduan</h2><p className="text-slate-500 text-sm mt-1">Isi detail kejadian dengan sebenar-benarnya.</p></div>
                  <div className="p-8"><form onSubmit={kirimLaporan} className="space-y-6"><div className="space-y-2"><label className="text-sm font-bold text-slate-700">Judul Laporan</label><input value={form.judul} onChange={(e)=>setForm({...form, judul:e.target.value})} className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#0f172a] outline-none transition-all bg-white/50" placeholder="Contoh: Kekerasan di Sekolah..." required/></div><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div className="space-y-2"><label className="text-sm font-bold text-slate-700">Kategori</label><select value={form.kategori} onChange={(e)=>setForm({...form, kategori:e.target.value})} className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#0f172a] outline-none bg-white/50"><option>Kekerasan Fisik</option><option>Kekerasan Seksual</option><option>Kekerasan Psikologis</option><option>Penelantaran</option><option>Lainnya</option></select></div><div className="space-y-2"><label className="text-sm font-bold text-slate-700">Tanggal Kejadian</label><input type="date" value={form.tanggal} onChange={(e)=>setForm({...form, tanggal:e.target.value})} className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#0f172a] outline-none bg-white/50" required/></div></div><div className="space-y-2"><label className="text-sm font-bold text-slate-700">Lokasi Kejadian</label><input value={form.lokasi} onChange={(e)=>setForm({...form, lokasi:e.target.value})} className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#0f172a] outline-none bg-white/50" placeholder="Nama Jalan, Gedung, dll..." required/></div><div className="space-y-2"><label className="text-sm font-bold text-slate-700">Kronologi Lengkap</label><textarea value={form.kronologi} onChange={(e)=>setForm({...form, kronologi:e.target.value})} className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#0f172a] outline-none h-32 resize-none bg-white/50" placeholder="Ceritakan detail kejadian..." required></textarea></div><div className="pt-4 flex items-center justify-end space-x-4"><button type="button" onClick={() => setView('home')} className="px-6 py-2 text-slate-500 font-bold hover:bg-slate-100 rounded-lg transition">Batal</button><button type="submit" className="px-6 py-2 bg-[#0f172a] hover:bg-slate-800 text-white font-bold rounded-lg shadow-md transition flex items-center"><Send size={18} className="mr-2"/> Kirim</button></div></form></div>
               </motion.div>
            )}
            ...
            {view === 'history' && (
               <motion.div key="history" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-6">
                 ...
                 {laporanUser.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 bg-white/80 backdrop-blur-md rounded-2xl border border-white/60 shadow-sm"><div className="bg-slate-100 p-6 rounded-full mb-4"><Search size={40} className="text-slate-400"/></div><h3 className="text-xl font-bold text-slate-700">Belum Ada Riwayat</h3><p className="text-slate-500 mt-2">Anda belum mengirimkan pengaduan apapun.</p><button onClick={() => setView('form')} className="mt-6 text-[#0f172a] font-bold hover:underline">Buat Pengaduan Pertama</button></div>
                 ) : (
                    <div className="grid grid-cols-1 gap-4">
                        {laporanUser.map((item) => (
                            <div key={item.id} className="bg-white/80 backdrop-blur-md p-6 rounded-2xl border border-white/60 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
                                ...
                                <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-2"><span className={`px-3 py-1 rounded-full text-xs font-bold uppercase border ${getStatusColor(item.status)}`}>{item.status || "Menunggu"}</span><span className="text-xs text-slate-400 flex items-center"><Calendar size={12} className="mr-1"/> {item.dibuatPada?.seconds ? new Date(item.dibuatPada.seconds * 1000).toLocaleDateString('id-ID') : 'Baru saja'}</span><span className="text-xs text-slate-400 flex items-center bg-slate-100 px-2 py-0.5 rounded cursor-pointer hover:bg-slate-200" onClick={() => {navigator.clipboard.writeText(item.id); alert('Token disalin!')}} title="Klik untuk salin Token"><Copy size={10} className="mr-1"/> ID Token: {item.id}</span></div>
                                    ...
                                    <div className="hidden md:flex flex-col items-end justify-center">
                                        ...
                                        <span className="text-xs text-slate-400 mt-2">{item.status === 'Selesai' ? 'Laporan Selesai' : item.status === 'Diproses' ? 'Sedang Ditangani' : 'Menunggu Petugas'}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
            ...
            {view === 'tracking' && (
               <motion.div key="tracking" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="max-w-xl mx-auto">
                  <div className="bg-white/80 backdrop-blur-md p-8 rounded-2xl shadow-lg border border-white/60 text-center">
                    ...
                    {trackResult && (
                        <div className="mt-6 text-left bg-slate-50/80 p-6 rounded-2xl border border-slate-200 animate-fade-in">
            ...
            {view === 'contact' && (
               <motion.div key="contact" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="bg-white/80 backdrop-blur-md p-8 md:p-10 rounded-2xl shadow-lg border border-white/60">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden">
                <div className="bg-emerald-500 p-6 text-center flex flex-col items-center justify-center">
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.2, type: "spring" }} className="h-20 w-20 bg-white rounded-full flex items-center justify-center text-emerald-500 shadow-lg mb-4"><Check size={48} strokeWidth={4} /></motion.div>
                    <h3 className="text-2xl font-bold text-white">Laporan Terkirim!</h3>
                    <p className="text-emerald-100 text-sm mt-1">Terima kasih atas laporan Anda.</p>
                </div>
                <div className="p-8">
                    <p className="text-slate-500 text-center mb-2 text-sm">Simpan Token ID ini untuk melacak status laporan Anda di menu "Lacak Pengaduan".</p>
                    <div className="bg-slate-100 border border-slate-200 rounded-xl p-4 flex items-center justify-between mb-6">
                        <span className="font-mono text-lg font-bold text-slate-700 tracking-wider truncate mr-4">{successModal.token}</span>
                        <button onClick={handleCopyToken} className={`p-2 rounded-lg transition-all flex items-center ${isCopied ? 'bg-emerald-100 text-emerald-600' : 'bg-white text-slate-500 hover:text-slate-800 shadow-sm'}`}>{isCopied ? <Check size={18} /> : <Copy size={18} />}</button>
                    </div>
                    <button onClick={closeSuccessModal} className="w-full py-3.5 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition shadow-lg">Tutup & Lihat Riwayat</button>
                </div>
            </motion.div>
        </motion.div>
      )}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden">
                <div className="p-6 text-center bg-red-50">
                    ...
                    <p className="text-slate-600 text-sm px-4">Apakah Anda yakin ingin keluar dari akun ini?</p>
                </div>
