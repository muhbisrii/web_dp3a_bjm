            ...
            {view === 'history' && (
               <motion.div key="history" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-6">
                 ...
                 {laporanUser.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 bg-white/80 backdrop-blur-md rounded-2xl border border-white/60 shadow-sm text-center px-4"><div className="bg-slate-100 p-6 rounded-full mb-4"><Search size={40} className="text-slate-400"/></div><h3 className="text-xl font-bold text-slate-700">Belum Ada Riwayat</h3><p className="text-slate-500 mt-2 text-sm">Anda belum mengirimkan pengaduan apapun.</p><button onClick={() => setView('form')} className="mt-6 text-[#0f172a] font-bold hover:underline text-sm">Buat Pengaduan Pertama</button></div>
                 ) : (
                                ...
                                <div className="flex flex-col gap-3">
                                    ...
                                    {item.tanggapanPetugas && (
                                        <div className="mt-2 bg-blue-50 p-3 rounded-xl border border-blue-100">
                                            <p className="text-[10px] font-bold text-blue-600 flex items-center mb-1"><MessageSquare size={10} className="mr-1"/> TANGGAPAN PETUGAS</p>
            ...
            {view === 'tracking' && (
               <motion.div key="tracking" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="max-w-xl mx-auto">
                  <div className="bg-white/80 backdrop-blur-md p-6 md:p-8 rounded-2xl shadow-lg border border-white/60 text-center">
                    ...
                    <form onSubmit={handleLacak} className="mb-6"><div className="relative"><input type="text" value={searchToken} onChange={(e) => setSearchToken(e.target.value)} placeholder="Tempel Token ID..." className="w-full p-3 md:p-4 pr-12 border border-slate-300 rounded-xl focus:ring-2 focus:ring-[#0f172a] outline-none text-center font-mono text-base md:text-lg bg-white/50" required /><Search className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400"/></div><button type="submit" disabled={trackLoading} className="w-full mt-4 py-3 bg-[#0f172a] text-white font-bold rounded-xl hover:bg-slate-800 transition shadow-md text-sm md:text-base">{trackLoading ? 'Mencari...' : 'Lacak Sekarang'}</button></form>
                    ...
                    {trackResult && (
                        <div className="mt-6 text-left bg-slate-50/80 p-5 rounded-2xl border border-slate-200 animate-fade-in">
                            ...
                            <div className="mt-4 pt-4 text-center"><button onClick={() => {setSearchToken(''); setTrackResult(null);}} className="text-xs md:text-sm text-slate-500 hover:text-[#0f172a] font-medium">Cari Laporan Lain</button></div>
                        </div>
                    )}
                  </div>
               </motion.div>
