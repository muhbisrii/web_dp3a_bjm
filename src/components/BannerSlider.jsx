...
import 'swiper/css/effect-fade'; // Efek fade agar transisi lebih modern
...
const banners = [
  {
    ...
    // Request: Tulisan di bawah untuk Slide 1
...
export default function BannerSlider() {
  return (
    // CONTAINER UTAMA (Ukuran tetap dipertahankan)
    <div className="w-full max-w-[1400px] mx-auto mt-0 md:mt-2 px-0 md:px-4">
      <Swiper
        spaceBetween={0}
        slidesPerView={1}
        loop={true}
        effect={'fade'} // Efek Fade lebih modern daripada geser biasa
      ...
      >
        {banners.map((banner) => {
          ...
          return (
            <SwiperSlide key={banner.id} className="bg-white">
              <Tag 
                {...props}
                // TINGGI TETAP (Tidak berubah)
                className={`block relative w-full h-[200px] md:h-[400px] flex items-center justify-center overflow-hidden ${isLink ? 'cursor-pointer' : 'cursor-default'}`}
              >
                {/* Gambar Background (Blur) - Efek modern untuk mengisi ruang kosong */}
                <div 
                ...
                {/* Hanya muncul di desktop/laptop agar tidak menutupi gambar di HP */}
                <div className="absolute inset-x-0 bottom-0 z-20 flex flex-col justify-end">
                  {/* Gradient halus di bawah */}
                  <div className="bg-gradient-to-t from-slate-900/90 via-slate-900/50 to-transparent p-6 md:p-8 pt-16">
                    
                    <div className="max-w-4xl mx-auto text-left">
                      {/* Judul Utama */}
                      <h3 className="text-white text-sm md:text-3xl font-bold mb-1 md:mb-2 drop-shadow-lg tracking-tight">
                        {banner.title}
                      </h3>

                      {/* Subjudul (Request User) */}
                      {banner.subtitle && (
                        <p className="text-slate-200 text-xs md:text-lg font-medium drop-shadow-md leading-relaxed border-l-4 border-indigo-500 pl-3">
                          {banner.subtitle}
                        </p>
                      )}
                    </div>

                    {/* Indikator Klik (Jika ada link) */}
                    {isLink && (
        ...
        <style jsx>{`
          .swiper-button-next, .swiper-button-prev {
            ...
            width: 44px;
            height: 44px;
            border-radius: 12px; /* Kotak rounded modern */
            border: 1px solid rgba(255,255,255,0.2);
            transition: all 0.3s ease;
          }
          .swiper-button-next:hover, .swiper-button-prev:hover {
            background: #4f46e5; /* Warna tema saat hover */
            border-color: #4f46e5;
            transform: scale(1.1);
